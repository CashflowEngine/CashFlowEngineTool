...
def page_monte_carlo(full_df):
    """Monte Carlo simulation page - OPTIMIZED with full stress test feedback."""
    ...
    with st.container(border=True):
        ...
        with c1:
            n_sims = st.number_input("Number of Simulations", value=5000, step=500, min_value=100, max_value=10000,
                help="More simulations = more accurate results, but slower. 5000 is recommended for stability.")
        with c2:
            sim_months = st.number_input("Simulation Period (Months)", value=60, step=6, min_value=1, max_value=120,
                help="How far into the future to simulate. 60 months (5 years) is a good baseline.")
        with c3:
            if from_builder and portfolio_daily_pnl is not None:
    ...
    if st.session_state.sim_run:
        if st.session_state.mc_results is None:
            ...
            with placeholder:
                ui.show_loading_overlay("Running Simulations", "Crunching thousands of possible futures...")
            time.sleep(0.1)
            ...
            try:
                ...
                if from_builder and portfolio_daily_pnl is not None:
                    ...
                    # For tracking worst days and theoretical WC
                    ...
                    for strat_name, group in active_df.groupby('strategy'):
                        ...
                        if stress_val > 0:
                            ...
                            if "Theoretical" in stress_mode and strat_name in stress_sel:
                                ...
                                if 'margin' in group.columns and 'legs' in group.columns:
                                    ...
                                    # Smart margin: only count PUT-side risk
                                    daily_put_margins = []
                    ...
                    # Create stress injections based on mode
                    if stress_val > 0:
                        ...
                        if "Historical" in stress_mode:
                            ...
                            if combined_wc < 0 and n_events > 0:
                                ...
                                debug_msg.append("---")
                                debug_msg.append(f"**Included strategies** ({len(stress_sel)} selected):")
                                for detail in strategy_wc_details:
                                    debug_msg.append(f"  - {detail}")
                            elif n_events > 0 and len(stress_sel) == 0:
                ...
                # Prepare stress injection parameters
                stress_inj_list = []
                ...
                st.session_state.mc_results = {
                    ...
                    'start_cap': start_cap, 'sim_months': sim_months,
                    'prob_profit': prob_profit, 'n_sims': n_sims,
        ...
        # Show stress test feedback if active
        n_stress = r.get('n_stress_per_sim', 0)
        n_years = max(1, r.get('sim_months', 12) / 12)
        ...
        with st.container(border=True):
            ...
            with k1:
                ui.render_hero_metric("Avg. Net Profit", f"${r['profit']:,.0f}", "Mean Result",
                    "hero-teal" if r['profit'] > 0 else "hero-coral",
                    tooltip="Average ending portfolio value minus initial capital across all simulations")
            with k2:
                ui.render_hero_metric("CAGR", f"{r['cagr']*100:.1f}%", "Ann. Growth",
                    "hero-teal" if r['cagr'] > 0 else "hero-coral",
                    tooltip="Compound Annual Growth Rate based on average ending value")
            with k3:
                ui.render_hero_metric("Expected DD", f"{r['dd_mean']*100:.1f}%", "Avg. Drawdown",
                    "hero-coral",
                    tooltip="Average maximum drawdown across all simulations - the expected worst-case decline")
            with k4:
                ui.render_hero_metric("MAR Ratio", f"{r['mar']:.2f}", "CAGR/DD",
                    "hero-teal" if r['mar'] > 1 else "hero-coral",
                    tooltip="CAGR divided by Expected Drawdown. Above 0.5 is acceptable, above 1.0 is good")
            with k5:
                ui.render_hero_metric("Prob. of Profit", f"{r['prob_profit']*100:.1f}%", f"Out of {r['n_sims']:,} sims",
                    "hero-teal" if r['prob_profit'] > 0.9 else ("hero-coral" if r['prob_profit'] < 0.7 else "hero-neutral"),
        ...
        with st.container(border=True):
            ...
            # Find best/worst within the stored paths
        ...
        with dist_col1:
            with st.container(border=True):
                ...
                fig_ret.add_vline(x=p5_ret, line_dash="dash", line_color="red",
                    annotation_text=f"P5: {p5_ret:.1f}%", annotation_position="top left")
                fig_ret.add_vline(x=p50_ret, line_dash="dash", line_color="blue",
                    annotation_text=f"P50: {p50_ret:.1f}%", annotation_position="top")
                fig_ret.add_vline(x=p95_ret, line_dash="dash", line_color="green",
                ...
                st.plotly_chart(fig_ret, use_container_width=True, key="mc_return_dist")
        ...
        with dist_col2:
            with st.container(border=True):
                ...
                fig_dd.add_vline(x=d5_pct, line_dash="dash", line_color="red",
                    annotation_text=f"P5: {d5_pct:.1f}%", annotation_position="top left")
                fig_dd.add_vline(x=d50_pct, line_dash="dash", line_color="blue",
                    annotation_text=f"P50: {d50_pct:.1f}%", annotation_position="top")
                fig_dd.add_vline(x=d95_pct, line_dash="dash", line_color="green",
                ...
                st.plotly_chart(fig_dd, use_container_width=True, key="mc_dd_dist")
